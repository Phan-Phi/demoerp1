...
export default function ColumnPrint(props: any) {
  const {
    type,
    data,
    maxHeight,
      ...
      return [
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original.sku");
            return <WrapperTableCell>{value}ssssss</WrapperTableCell>;
          },
        },
        {
          Header: <FormattedMessage id={`table.productName`} />,
    ...
    if (type === "sale") {
      ...
      return [
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            ...
            return (
              <WrapperTableCell fontWeight={700}>
                {"SL mặt hàng: "}
                <NumberFormat value={get(row, "original[0].count")} suffix="" />
              </WrapperTableCell>
            );
          },
        },
        ...
        },
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_quantity");
            return (
        ...
        },
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_revenue_incl_tax");
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat value={parseFloat(value)} />
              </WrapperTableCell>
        ...
        },
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_net_revenue_incl_tax");
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat value={parseFloat(value)} />
              </WrapperTableCell>
    ...
    if (type === "profit") {
      ...
      return [
        ...
        },
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_base_amount_incl_tax");
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat value={parseFloat(value)} />
              </WrapperTableCell>
            ...
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat
                  value={parseFloat(
                    (
                      get(row, "original[0].sum_net_revenue_incl_tax") -
                      get(row, "original[0].sum_base_amount_incl_tax")
                    ).toFixed(2)
                  )}
                />
              </WrapperTableCell>
            ...
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat
                  value={parseFloat(
                    (
                      ((parseFloat(get(row, "original[0].sum_net_revenue_incl_tax")) -
                        parseFloat(get(row, "original[0].sum_base_amount_incl_tax"))) /
                        parseFloat(get(row, "original[0].sum_net_revenue_incl_tax"))) *
                      100
                    ).toFixed(2)
                  )}
      ...
      return [
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].count");
            return (
              <WrapperTableCell fontWeight={700}>
                {"SL mặt hàng: "}
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            ...
            const currentQuantity =
              get(row, "original[0].sum_beginning_quantity") +
              get(row, "original[0].sum_input_quantity") -
              get(row, "original[0].sum_output_quantity");
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat value={currentQuantity} suffix="" />
              </WrapperTableCell>
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            ...
            const value = get(
              row,
              "original[0].total_current_price_incl_tax_till_date_end"
            );
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            ...
            const value = get(
              row,
              "original[0].total_current_base_price_incl_tax_till_date_end"
            );
    ...
    if (type === "import_export_stock") {
      ...
      return [
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_beginning_quantity");
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_beginning_amount");
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            ...
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat
                  value={get(row, "original[0].sum_input_quantity")}
                  suffix=""
                />
              </WrapperTableCell>
            );
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_total_input_amount");
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_output_quantity");
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].sum_total_output_amount");
            ...
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat
                  value={parseFloat(
                    (
                      parseFloat(get(row, "original[0].sum_beginning_quantity")) +
                      parseFloat(get(row, "original[0].sum_input_quantity")) -
                      parseFloat(get(row, "original[0].sum_output_quantity"))
                    ).toFixed(2)
                  )}
                  suffix=""
                />
            ...
            return (
              <WrapperTableCell fontWeight={700} textAlign="right">
                <NumberFormat
                  value={parseFloat(
                    (
                      parseFloat(get(row, "original[0].sum_beginning_amount")) +
                      parseFloat(get(row, "original[0].sum_total_input_amount")) -
                      parseFloat(get(row, "original[0].sum_total_output_amount"))
                    ).toFixed(2)
                  )}
                />
              </WrapperTableCell>
