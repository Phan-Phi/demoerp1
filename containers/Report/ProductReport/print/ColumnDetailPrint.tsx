...
export default function ColumnDetailPrint(props: any) {
  const {
    filter,
    type,
    data,
    ...
    if (type === "sale") {
      ...
      return [
        ...
        },
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].results[0].quantity_count");
            return (
      ...
      return [
        ...
        },
        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const quantity =
              get(row, "original[0].beginning_quantity") +
              get(row, "original[0].input_quantity") -
              get(row, "original[0].output_quantity");
            return (
              <WrapperTableCell textAlign="right" fontWeight={700}>
                {quantity}
              </WrapperTableCell>
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            ...
            return (
              <WrapperTableCell textAlign="right" fontWeight={700}>
                <NumberFormat
                  value={
                    parseFloat(get(row, "original[0].current_price.incl_tax")) * quantity
                  }
                />
              </WrapperTableCell>
            );
...

        {
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            const { row } = props;
            const value = get(row, "original[0].current_base_price.incl_tax");
          ...
          Cell: (props: PropsWithChildren<CellProps<any, any>>) => {
            ...
            return (
              <WrapperTableCell textAlign="right" fontWeight={700}>
                <NumberFormat
                  value={
                    parseFloat(get(row, "original[0].current_base_price.incl_tax")) *
                    quantity
                  }
                />
              </WrapperTableCell>
