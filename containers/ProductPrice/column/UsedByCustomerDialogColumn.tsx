...
type UsedByCustomerDialogColumnProps =
  CommonTableProps<ADMIN_CUSTOMER_CUSTOMER_VIEW_TYPE_V1> & Record<string, any>;

const UsedByCustomerDialogColumn = (props: UsedByCustomerDialogColumnProps) => {
  const {
    getTable,
    data,
    count,
  ...
  const columns = useMemo(() => {
    return [
...

      {
        ...
        accessor: "id",
        Cell: (
          props: PropsWithChildren<CellProps<ADMIN_CUSTOMER_CUSTOMER_VIEW_TYPE_V1, any>>
        ) => {
          const { row } = props;
...
export default UsedByCustomerDialogColumn;
